...
normalized = raw_input.lower().replace(" ", "").replace("-", "").replace(".", "").strip("_")
...
search_terms.append("".join(parts[i:j])) # ترکیب‌های متوالی
...
return list(dict.fromkeys([underscored, key.replace("", " ").replace(" ", "")]))
